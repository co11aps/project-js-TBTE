...
function templateBookCard({_id, author, contributor, title, book_image, list_name}){
...
function templateBookCategoryEl({books}){
    ...
    const result = `<li class="book-item-home">
                    <h3 class="book-category-title">${bookCategory}</h3>
                    <ul class="books-category-list">` + bookCards +
                    `</ul>
...
function templateHomeBookMarkup(dataPopularBooks){
    ...
    const result = `<h2 class="books-home-title">Best Sellers <span class="books-home-title-span">Books</span></h2>` + 
                 `<ul class="books-home-list">` + markup + `</ul>`;
