...
async function getFormValues(event) {
    ...
    try {
        ...
        const weatherHtml = `
                ...
                <p class="weather-city-info">Temperature: ${Math.floor(temperature)} &deg;C</p>
                <p class="weather-city-info">${text}</p>
        `;
        weatherInfo.innerHTML = weatherHtml;
        if (temperature < 0 || text.toLowerCase().includes('cloudy') || text.toLowerCase().includes('rain')) {
            iziToast.info({
...
async function serviceWeather(city) {
    ...
    const response = await (await axios.get(`${BASE}/current.json`, {
        params: {
